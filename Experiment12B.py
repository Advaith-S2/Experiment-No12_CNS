...
def vigenere_encrypt(text, key):
    ...
    for char in text:
        if char.isalpha():
            ...
            shift = (
                ord(key[key_index % len(key)]) - ord('A')
            )
...
if (x - y) % m == 0:
    print(
        f"{x} is congruent to {y} modulo {m}"
    )
else:
    print(
...
if result == 1:
    ...
    print(
        "Fermat's Little Theorem verification failed."
    )
...
# ASCII text is used instead of the Unicode congruence symbol.
print("x == 2 (mod 3)")
print("x == 3 (mod 5)")
print("x == 2 (mod 7)")
...
for r, modulus in zip(
    remainders,
    moduli
):
    print(
        f"x mod {modulus} = "
        f"{crt_result % modulus}"
    )
...
tests = [
    (
        "Caesar Cipher",
        caesar_decrypted == plaintext
    ),
    (
        "Affine Cipher",
        affine_decrypted == plaintext
    ),
    (
        "Vigenere Cipher",
        vigenere_decrypted == plaintext
    ),
    (
        "AES-CBC",
        aes_decrypted == plaintext
    )
]
...
The Chinese Remainder Theorem demonstrates solving a system
of simultaneous modular equations.

AES-CBC demonstrates modern symmetric encryption using an
Initialization Vector (IV) and block chaining.

All encryption and decryption tests were successfully verified.
""")